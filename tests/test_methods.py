...
def test_parametric():
    ...
    expected_results = np.array([[-21.5224678, -32.51123235, -21.5224678, -32.51123235, -19.0, -19.0]])

    # Call the function with the test data
    results = parametric(test_pnl, test_alpha, daily_std)

    # Test whether the results are as expected
    np.testing.assert_array_almost_equal(results.data, expected_results, decimal=4)
...
def test_historic():
    ...
    # Test whether the results are as expected
    np.testing.assert_array_almost_equal(results.data, expected_results, decimal=5)
